 ...
 */
@Deprecated
public interface FluentFunction_OLD {
     ...
     */
    List<FluentValue<?>> apply(ResolvedParameters_OLD parameters, Scope scope);
    ...
    default List<FluentValue<?>> select(final SelectExpression selectExpression,
                                        final ResolvedParameters_OLD parameters,
                                        final Scope scope) {
        throw new UnsupportedOperationException("select - old -- not supported. todo");
        /*
        // this should work for most functions ... unless they reduce!
        return apply( parameters, scope ).stream()
                .map( v -> FluentValueFormatter.select( v,selectExpression, parameters, scope ) )
     ...
     */
    static void ensureInput(final ResolvedParameters_OLD rp) {
        if (rp.noPositionals()) {
            throw FluentFunctionException.create( "No positionals arguments supplied; at least one is required." );
        }
    }
    ...
    /**
     * Utility method: get the specific FluentValue subclass from FluentValue or throw an Exception.
     *
    ...
    static List<FluentValue<?>> mapOverNumbers(final Stream<FluentValue<?>> stream,
        ...
        return stream.<FluentValue<?>>map(
                value -> FluentFunction_OLD.applyIfNumber( value, scope, fn )
                ).toList();
    }
    ...
    static void validate(FluentValue<?> in) {
        if (in instanceof FluentError(String value)) {
            throw FluentFunctionException.create( value );
        }
    }
