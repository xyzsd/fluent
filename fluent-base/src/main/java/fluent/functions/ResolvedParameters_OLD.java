 ...
 */
public class ResolvedParameters_OLD {
    ...
    //          most of the time positionals arguments are just iterated through in-order
     ...
     * Empty
     */
    public static final ResolvedParameters_OLD EMPTY = new ResolvedParameters_OLD( List.of(), Options.EMPTY );
    ...
    // positionals arguments. single-items are in single-item lists (via List.of())
    // e.g. FUNCTION("hello", "goodbye") :
    //      "hello" would be the first positionals, and would be a single item list
    //      "goodbye" is the second positionals, and would also be a single item list
    // so this would be List( List(FluentString("hello")), List(FluentString("goodbye")) )
    // A list often results from the application of a function which returns a list
    ...
    public static ResolvedParameters_OLD from(final List<List<FluentValue<?>>> listIn, final Options opts) {
        ...
        return new ResolvedParameters_OLD( collapsed, opts );
    }


    // typically used by FluentValue<> formatters
    // uses options supplied in scope -- if any
    public static ResolvedParameters_OLD from(FluentValue<?> value, final Scope scope) {
        return new ResolvedParameters_OLD( List.of(List.of(value)), scope.options() );
    }

    public static ResolvedParameters_OLD from(List<FluentValue<?>> list, final Scope scope) {
        if(list.isEmpty()) {
            return new ResolvedParameters_OLD( List.of(), scope.options() );
        }
        return new ResolvedParameters_OLD( List.of(List.copyOf(list)), scope.options() );
    }


    // replace options with new options
    public ResolvedParameters_OLD with(Options options) {
        return new ResolvedParameters_OLD( this.pos, Objects.requireNonNull( options ) );
    }


    private ResolvedParameters_OLD(List<List<FluentValue<?>>> list, Options opts) {
        this.pos = list;
        this.options = opts;
    }
    ...
    /**
     * Count the number positionals arguments.
     */
